...
**Phase 5b's algorithm and code are complete and covered by CPU-level
tests, including this fix wave's new tests -- but the GPU session's
measured throughput and correctness-gate numbers are WITHDRAWN, pending
a root-cause investigation.** Do not cite the +21.8%/+76.9% throughput
figures or either "correctness gate passed" claim from the original
session below; both are unreliable. `make check` green throughout (158
tests, 2 skipped, 2 deselected; lint and `mypy --strict` clean).

**What the final whole-branch review found**: the GPU session's own
baseline (`--drafter none`) generated a degenerate output -- 64
repetitions of token id `0`, for all 4 test prompts, not a plausible
continuation of any of them. Both correctness gates "passed" only
because they matched this same degenerate baseline at k=4/k=8 -- a
vacuous pass, since two broken outputs agreeing proves nothing about
correctness. The structural reason this could happen at all: the
baseline was generated by `run_speculative_rounds` itself (called with
`drafter=None`), the exact shared propose/verify/accept/rollback code
the gates exist to check, so the gate could not in principle catch a bug
living in that shared code. The k=1/k=2 sweep runs -- the only runs that
session produced plausible, varied tokens on -- were never checked
against the baseline at all. Full account, including every withdrawn
number kept for the record and clearly relabeled:
`docs/findings/2026-09-17-phase-5b-speculative-decoding-run.md`.

**What this fix wave added, 2026-09-17, to close the gap**: a genuinely
independent CPU-level oracle (`plain_greedy_generate` in `decode.py`,
sharing no code with `run_speculative_rounds`) now backs
`scripts/run_speculative_bench.py`'s `--drafter none` baseline path;
runtime cache-lag assertions in both `run_speculative_rounds` and
`DraftModelDrafter.propose` turn a future silent cache desync into a
crash instead of plausible-looking garbage; and a new real-model,
real-tokenizer, real-`transformers.Cache` end-to-end test exercises
`DraftModelDrafter` inside the full loop for the first time outside the
now-discredited GPU gate -- and passes. All three are CPU-only,
deterministic, reproducible without spending GPU money, and now part of
`make check`.

**What's still open, and explicitly not this fix wave's job**:
root-causing why the GPU session's baseline degenerated needs a new
rented GPU session (real money) and is a decision for the project owner,
not something to guess at from a laptop. A trustworthy Phase 5b
throughput number needs that root-cause work done first, then a re-run
checking `--compare-generated-tokens` at every k-sweep point, not just
k=4.

**Memory checkpoints** (unaffected by the above -- real allocator
readings, not token-correctness-dependent) confirmed the design's own
budget rationale live: quantized target alone allocates 16.75GB, target
plus bf16 draft model together 29.62GB -- both well within a single L40,
no OOM at any point in the session.

**Total Phase 5b GPU cost: $0.6833** of the $10 cap (6.83%), pod
`2t6wh9l3okl3gj`, L40, $0.82/hr, 3000s (a real dollar figure, unaffected
by the withdrawal above) -- one session covering both now-discredited
correctness gates, the now-withdrawn three-way measured run, and the
now-withdrawn 8-run k-sweep.

## Next step

**Phase 5b's algorithm and code are done and reviewed; a GPU
re-investigation is needed before its performance claims can be
trusted.** That re-investigation -- root-causing the degenerate
baseline, then a clean re-measured run with `--compare-generated-tokens`
checked at every k -- is not done here, is out of scope for the fix wave
that closed this review, and needs a new rented GPU session (real
money), a decision for the project owner. Phase 4 is merged (PR #4).
Phase 5a is complete and ready to push as a PR; Phase 5b is ready to push
as its own PR per the one-branch-per-phase convention, but its own PR
description should carry this withdrawal forward, not the original
session's headline numbers. Phase 3's PR (#3) already merged. Phase 2's
PR is still open and awaiting maintainer review; no further work planned
on it beyond responding to review feedback. **Phase 6 (final benchmark
vs. vLLM/SGLang) planning can proceed in parallel, but must not cite
Phase 5b's withdrawn numbers.**
